...
function __strToBytes(str, unicode) {
    ...
    var data = [];
    for (var i = 0; i < str.length; i++) { data[i] = str.charCodeAt(i); }
...
function __encrypt(data) {
    ...
    for (var i = 1; i <= __pos; i++) { __plain[i] = __rand() & 0xFF; }
    __pos++;

    for (var i = 0; i < 8; i++) { __prePlain[i] = 0; }
...
function __decrypt(data) {
	...
	*/
    ...
    while (padding <= 2) {
        ...
        if (__pos == 8) {
            m = data;
            if (!__decrypt8Bytes()) { return null; }
        }
    }
	...
	*/
    for (padding = 1; padding < 8; padding++) {
        if (__pos < 8) {
            if ((m[__preCryptPos + __pos] ^ __prePlain[__pos]) != 0) { return null; }
            __pos++;
        }
        if (__pos == 8) {
            m = data;
...
function __encrypt8bytes() {
    for (var i = 0; i < 8; i++) {
        if (__header) { __plain[i] ^= __prePlain[i]; } else { __plain[i] ^= __out[__preCryptPos + i]; }
    }
...
function __dataFromStr(str, isASCII) {
    var data = [];
    if (isASCII) {
        for (var i = 0; i < str.length; i++) { data[i] = str.charCodeAt(i) & 0xff; }
    } else {
        var k = 0;
...
var TEA = {
    ...
    enAsBase64 (str, isASCII) { // output base64 encoded
        var data = __dataFromStr(str, isASCII);
        var encrypted = __encrypt(data);
        var bytes = '';
        for (var i = 0; i < encrypted.length; i++) { bytes += String.fromCharCode(encrypted[i]); }
